...
void Level::loadMap(std::string mapName, Graphics& graphics) {
				...
				while (pData) {
					...
					if (pTile != NULL) {
						...
						while (pTile) {
							...
							Vector2 finalTilesetPosition = this->getTilesetPosition(tls, gid,
	...
	if (pObjectGroup != NULL) {
		while (pObjectGroup) {
			...
			// Other object gorup goes here with an else if (ss.str() == "whatever")
				...
				if (pObject != NULL) {
					while (pObject) {
						...
						if (pPolyline != NULL) {
							...
							// Now we have each of the pairs. Loop thorugh the list of pairs
						...
						// Parsing out slopes that have multiple points
...
std::vector<Rectangle> Level::checkTileCollisions(const Rectangle& other) {
	...
	for (int i = 0; i < this->_collisionRects.size(); i++) {
		if (this->_collisionRects.at(i).collideWith(other)) {
